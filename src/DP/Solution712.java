...
public class Solution712 {
    ...
    int dp(String s1, int i, String s2, int j) {
        ...
        if(ch1 == ch2) {
            // if the current characters are the same, the state is equal to the dp[i+1][j+1],
            // because this path will delete the character, but the min on the second part will 
            // do the selection of the path.
            memo[i][j] = dp(s1, i+1, s2, j+1);
        }
        else {
            memo[i][j] = Math.min(dp(s1, i, s2, j+1) + ch2, dp(s1, i+1, s2,j) + ch1);
        }
