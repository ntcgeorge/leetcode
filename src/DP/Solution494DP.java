...
/**
 * link: https://leetcode.cn/problems/target-sum/submissions/529040622/
 * time: 30 min
 * notes:
 * 1. the DP version of 494
 * 2. finding the target is equal to finding the subset whose sum is (total_sum + target) / 2
 * 3. refer to Solution416 which provides the solution to find the subset   
 */
public class Solution494DP {
    ...
    public static void main(String[] args) {
        ...
        int n = fr.nextInt();
        for(int i=0; i < n; i++) {
            Solution494DP sol = new Solution494DP();
            int[] nums = fr.readIntLine(",");
            int target = fr.nextInt();
            out.println(sol.findTargetSumWays(nums, target));
        }
        out.close();
    }
    int[][] memo; // memo[i][j] = x: there are x ways selecting subset in the first i-th elements such that the sum is j;
