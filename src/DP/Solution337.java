...
public class Solution337 {
    ...
    public static void main(String[] args) {
        ...
        int n = fr.nextInt();
        for(int i=0; i < n; i++) {
            TreeNode node = new TreeNode(fr.next().split(","));
            Solution337 sol = new Solution337();
            out.println(sol.rob(node));
        }
        out.close();
    }
    ...
    public int rob(TreeNode root) {
        memo = new HashMap[2];
        memo[0] = new HashMap<TreeNode, Integer>();
        memo[1] = new HashMap<TreeNode, Integer>();
        build(root);
    ...
    private int dp(TreeNode node, int robbed) {
        if(node == null) return 0;
        if(memo[robbed].get(node) != -100) return memo[robbed].get(node);
        if(robbed == 1) return dp(node.left, 0) + dp(node.right, 0) + node.val;
        int val =  max4(
                dp(node.left, 1) + dp(node.right, 1),
                dp(node.left, 1) + dp(node.right, 0),
                dp(node.left, 0) + dp(node.right,1),
                dp(node.left, 0) + dp(node.right, 0)
            );
        memo[robbed].put(node, val);
        return val;
    }
